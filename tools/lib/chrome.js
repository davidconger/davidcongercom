/**
 * The chrome every page of the site shares: the pinned utility bar with the
 * social icons, the masthead, and the footer.
 *
 * It lives here rather than in each generator because four different things
 * emit it -- the year pages, the gallery index, the ~1,600 individual gallery
 * pages and the home page -- and the whole point of the redesign is that they
 * are the same page furniture at different depths. The only thing that varies
 * is how far up the site root is and what the left-hand control does.
 */

/* Fluent's home glyph, regular and filled, swapped on hover exactly as the mail
   icon is -- so the one navigation control on the left of the bar reads as part
   of the same set as the social links rather than as leftover text. */
...
${SOCIAL.split('\n').map(function (l) { return l ? '\t\t' + l.replace(/^\t\t/, '') : l; }).join('\n')}
	</div>
</div>`;
}

/** The Home control, for every page that is not the home page. */
function homeLink(up) {
	return `			<a class="socialLink socialHome" href="${up}index.htm" aria-label="Home" title="Home">${HOME_ICON}</a>`;
}

/** The full masthead. Hidden by the stylesheet once the compact one takes over. */
function masthead() {
	return `<div class="headerText streamHeader">
...
/* The year is fixed rather than generated from the clock: the copyright covers
   the archive, and a date that moves on its own is a date nobody checked. */
function footer() {
	return `<p class="siteFooter streamFooter">
